...
# Compare between fixed-effect block model AND the random-effect block model of the following:
- ANOVA table p-values : when we treat block as random in the rcbd model, then the p-values are lower (p = 161e-06) than the p-values (p= 0.000505) in fixed block rcbd model.
- emmeans SE : The SE of emmeans of random block rcbd model is higher (330) than the fixed block rcbd model which is 307.
- letter separation : No difference observed in letter separation.
